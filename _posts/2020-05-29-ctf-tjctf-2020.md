...
This challenge gives a binary that accepts four user input.
...
The binary asks for one user input. A quick fuzz gives us the offset we need to control the instruction pointer.
...
The binary asks for two user input.
...
Our input (AAAA or 0x41414141) can be seen 7th output.
...
payload += "AAAA%8$s %69$pBBBB" # Will leak the value of printf libc and a stack address
```

After leaking the address, we can use [LIBC Database](https://libc.blukat.me/) to determine the version of the LIBC used.

For the stage 2 payload, we need to overwrite the location of the return address with the `main()` address to loop back, and overwrite the GOT value of `printf()` with `system()` so that instead of printing our input "/bin/sh", it will be executed by the system function.
